...
`npm run start` serves everything from a single process at
http://localhost:8790 and opens it in your default browser (best-effort —
if it doesn't open, that URL is where the game is). `npm run dev` plays at
the port Vite prints (5173, or 5174+ if 5173 is taken), proxying `/api`
through to the API on port 8790.
...
First run only: `npx playwright install chromium`. The e2e client runs on
...
`scripts/build-data.ts` for the exact filenames; copies live in the n2-prep
repo's `data/raw/`). The build is deterministic: identical inputs produce
byte-identical output. Generated files are committed, so you only need this
